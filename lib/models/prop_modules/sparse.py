...
class SparseNormConv(nn.Module):
    def __init__(self, cfg):
        super(SparseNormConv, self).__init__()
        self.num_scale_layers = cfg.NUM_SCALE_LAYERS
        self.hidden_size = cfg.HIDDEN_SIZE
        ...
        for scale_idx, num_layer in enumerate(self.num_scale_layers):
            ...
            scale_layers.extend([first_layer] + rest_layers)
            self.layers.append(scale_layers)
            norm_layers = nn.ModuleList([nn.BatchNorm1d(self.hidden_size) for _ in range(1, num_layer)])
            self.norms.append(norm_layers)
    ...
    def forward(self, x):
        ...
        for scale_idx, scale_layers in enumerate(self.layers):
            ...
            for i, layer in enumerate(scale_layers):
                ...
                map_h[:, :, scale_s_idxs, scale_e_idxs] = x
                map_mask[:, :, scale_s_idxs, scale_e_idxs] = 1
                if i != len(scale_layers) - 1:
                    x = self.norms[scale_idx][i](x)
                    x = torch.tanh(x)
...
class SparseMoveConv(nn.Module):
    def __init__(self, cfg):
        super(SparseMoveConv, self).__init__()
        self.num_scale_layers = cfg.NUM_SCALE_LAYERS
        self.hidden_size = cfg.HIDDEN_SIZE

        self.layers = nn.ModuleList()
        self.movement_layers = nn.ModuleList()
        ...
        for scale_idx, num_layer in enumerate(self.num_scale_layers):
            ...
            movement_layer = nn.ModuleList(
                [nn.Conv1d(self.hidden_size, self.hidden_size, 3, 1, padding=1) for _ in range(num_layer)])
            self.movement_layers.append(movement_layer)
    ...
    def forward(self, x):
        ...
        for scale_idx, scale_layers in enumerate(self.layers):
            ...
            for i, layer in enumerate(scale_layers):
                # improvement
                move = self.movement_layers[scale_idx][i](x)
                move = torch.relu(move)
                x = x + move
